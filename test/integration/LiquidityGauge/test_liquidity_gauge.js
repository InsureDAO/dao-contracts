...
describe('LiquidityGauge', function() {
    ...
    beforeEach(async () => {
        ...
        liquidity_gauge = await LiquidityGauge.deploy(mock_lp_token.address, minter.address, alice.address);
    });
    ...
    describe("test_liquidity_gauge", function(){
        ...
        it("test_gauge_integral", async()=>{
            ...
            await mock_lp_token.transfer(bob.address, (await mock_lp_token.balanceOf(alice.address)).div(BigNumber.from("2")));
            ...
            for(let i=0; i<10; i++){
                ...
                if(is_withdraw){
                    //withdraw
                    let amount = BigNumber.from(Math.floor(Math.random()*10000).toString()).mul(await liquidity_gauge.balanceOf(bob.address)).div(BigNumber.from('10000'));
                    ...
                    //deposit
                    let amount = BigNumber.from(Math.floor(Math.random()*10000).toString()).mul(await mock_lp_token.balanceOf(bob.address)).div(BigNumber.from('10')).div(BigNumber.from('10000'));
                ...
                if(is_alice){
                    ...
                    if(is_withdraw_alice){
                        ...
                        let amount_alice = BigNumber.from(Math.floor(Math.random()*10000).toString()).mul(await liquidity_gauge.balanceOf(alice.address)).div(BigNumber.from('10')).div(BigNumber.from('10000'));
                        ...
                        let amount_alice = BigNumber.from(Math.floor(Math.random()*10000).toString()).mul(await mock_lp_token.balanceOf(alice.address)).div(BigNumber.from('10000'));
                ...
                await liquidity_gauge.user_checkpoint(alice.address);
                await update_integral();
                console.log(i +":"+ dt/86400 +":"+ integral +":"+ (await liquidity_gauge.integrate_fraction(alice.address))); //approx 1e-20

            }
        });

        it("test_mining_with_votelock", async()=>{
            await ethers.provider.send("evm_increaseTime", [WEEK.mul(BigNumber.from('2')).add(BigNumber.from('5')).toNumber()]);
            ...
            expect(rewards_alice.mul(BigNumber.from('10000000000000000')).div(rewards_bob)).to.equal(BigNumber.from('25000000000000000'));//approx = 1e-16
