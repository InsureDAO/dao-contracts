...
describe('GaugeController', function(){
    ...
    beforeEach(async () => {
        ...
        lg1 = await LiquidityGauge.deploy(mock_lp_token.address, minter.address, creator.address);
        lg2 = await LiquidityGauge.deploy(mock_lp_token.address, minter.address, creator.address);
        lg3 = await LiquidityGauge.deploy(mock_lp_token.address, minter.address, creator.address);
    ...
    describe("test_vote", function(){
        ...
        it("test_vote_change", async()=>{
            await gauge_controller.vote_for_gauge_weights(three_gauges[1], 1234);
            await expect(gauge_controller.vote_for_gauge_weights(three_gauges[1], 1234)).to.revertedWith("Cannot vote so often");
        ...
        it("test_vote_no_balance", async()=>{
            await expect(gauge_controller.connect(alice).vote_for_gauge_weights(three_gauges[0], 10000)).to.revertedWith("Your token lock expires too soon");
        });
        ...
        it("test_vote_expired", async()=>{
            ...
            await expect(
                gauge_controller.vote_for_gauge_weights(three_gauges[0], 10000)).to.revertedWith(
        ...
        it("test_invalid_gauge_id", async()=>{
            await expect(
                gauge_controller.vote_for_gauge_weights(three_gauges[2], 10000)).to.revertedWith(
                "Gauge not added"
            );
        });
        ...
        it("test_over_user_weight", async()=>{
            await expect(
                gauge_controller.vote_for_gauge_weights(three_gauges[0], 10001)).to.revertedWith(
        ...
        it("test_over_weight_multiple", async()=>{
            ...
            await expect(
                gauge_controller.vote_for_gauge_weights(three_gauges[1], 4000)).to.revertedWith(
                "Used too much power"
            );
        });
        ...
        it("test_over_weight_adjust_existing", async()=>{
            ...
            await expect(
                gauge_controller.vote_for_gauge_weights(three_gauges[0], 8000)).to.revertedWith(
                "Used too much power"
            );
        });
