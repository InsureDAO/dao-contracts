...
describe('InsureToken', function(){
    describe("test_setters", function(){
        ...
        it("test_set_minter_admin_only", async()=>{
            await expect(Insure.connect(alice).set_minter(bob.address)).to.revertedWith("dev: admin only");
        });

        it("test_set_admin_admin_only", async()=>{
            await expect(Insure.connect(alice).set_admin(bob.address)).to.revertedWith("dev: admin only");
        });

        it("test_set_name_admin_only", async()=>{
            await expect(Insure.connect(alice).set_name("Foo Token", "FOO")).to.revertedWith("Only admin is allowed to change name");
        });
