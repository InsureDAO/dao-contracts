...
describe('VestingEscrow', function() {
    ...
    describe("test_fund", function(){
        beforeEach(async () => {
            ...
            vesting = await VestingEscrow.deploy(coin_a.address, start_time, end_time, true, [alice.address, bob.address, chad.address, tom.address]);
        ...
        it("test_initial_locked", async()=>{
            ...
            let i;
            for(i=1; i<100; i++){
                if(i<10){recipients.push(accounts[i]);}else{recipients.push(ZERO_ADDRESS);}
            }
            allocation = [ten_to_the_17];
            let t;
            for(t=1; t<100; t++){
        ...
        it("test_partial_recipients", async()=>{
            ...
            let i;
            for(i=1; i<100; i++){
                if(i<5){recipients.push(accounts[i]);}else{recipients.push(ZERO_ADDRESS);}
            }
            allocation = [ten_to_the_17];
            let t;
            for(t=1; t<100; t++){
        ...
        it("test_multiple_calls_different_recipients", async()=>{
            ...
            let i;
            for(i=1; i<100; i++){
                if(i<1){recipients.push(accounts[i]);}else{recipients.push(ZERO_ADDRESS);}
            }
            allocation = [ten_to_the_20];
            let t;
            for(t=1; t<100; t++){
                if(t<2){allocation.push(allocation[t-1].add(ten_to_the_20));}else{allocation.push(BigNumber.from('0'));}
            }
        ...
        it("test_admin_only", async()=>{
            ...
            await expect(
                vesting.connect(seven).fund(recipients, allocation)).to.revertedWith(
                "dev admin only"
            );
        });
        ...
        it("test_fund_admin", async()=>{
            ...
            let i;
            for(i=1; i<100; i++){
                if(i<1){recipients.push(accounts[i].address);}else{recipients.push(ZERO_ADDRESS);}
            }
            allocation = [ten_to_the_20];
            let t;
            for(t=1; t<100; t++){
            ...
            for(let x = 0; x<10; x++){
                if(x<5){
                    ...
                    await expect(
                        vesting.connect(accounts[x]).fund(recipients, allocation)).to.revertedWith(
                        "dev admin only"
                    );
                }
            }
        ...
        it("test_disable_fund_admin", async()=>{
            ...
            await expect(
                vesting.connect(accounts[3]).fund(recipients, allocation)).to.revertedWith(
