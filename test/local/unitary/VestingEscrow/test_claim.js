...
describe("VestingEscrow", function () {
  ...
  before(async () => {
    ...
    vesting = await VestingEscrow.deploy(
      coin_a.address,
      start_time,
      end_time,
      true,
      [alice.address, bob.address, chad.address, tom.address]
    );
    ...
    expect(await vesting.unallocated_supply()).to.equal(
      ten_to_the_21.sub(ten_to_the_20)
    );
  });
  ...
  describe("test_claim", function () {
    ...
    it("test_claim_partial", async () => {
      ...
      expect(await vesting.total_claimed(alice.address)).to.equal(
        expected_amount
      );
    });
    ...
    it("test_claim_multiple", async () => {
      ...
      await ethers.provider.send("evm_increaseTime", [
        start_time.sub(timestamp).sub("1000").toNumber(),
      ]);
      ...
      for (i = 0; i < 11; i++) {
        await ethers.provider.send("evm_increaseTime", [
          end_time.sub(start_time).div("10").or("0").toNumber(),
        ]);
        await vesting.connect(alice).claim(alice.address);
