...
describe('PoolProxy', () => {
    ...
    describe("add_distributor", function () {
        it("only_ownership_admin", async () => {
            await expect(pool_proxy.connect(alice).add_distributor(tokenA.address, "dstrA for tokenA", dstrA.address)).to.revertedWith("only ownership admin");
        });

        it("token cannot be zero address", async()=>{
            await expect(pool_proxy.add_distributor(ZERO_ADDRESS, "dstrA for zero_address", dstrA.address)).to.revertedWith("_token cannot be zero address");
        });
    ...
    describe("set_distributor_weight", function(){
        ...
        it("cannot set weight for not added ID", async()=>{
            await expect(pool_proxy.connect(alice).set_distributor_weight(tokenA.address, 0, 1000)).to.revertedWith("not added yet");
        });
    ...
    describe("set_distributor_weight_many", function(){
        it("only parameter admin", async()=>{
            ...
            await expect(pool_proxy.set_distributor_weight_many(tokens, ids, weights)).to.revertedWith("only parameter admin");
        });
    ...
    describe("set_distributor", function(){
        it("only admin", async()=>{
            await pool_proxy.add_distributor(tokenA.address, "dstrA for tokenA", dstrA.address);
            await expect(pool_proxy.connect(alice).set_distributor(tokenA.address, 0, "tokenA dstr V2", dstrB.address)).to.revertedWith("only ownership admin");
        });

        it("cannot set to not added purpose", async()=>{
            await expect(pool_proxy.set_distributor(tokenA.address, 0, "tokenA dstr v2", dstrB.address)).to.revertedWith("not added yet");
        });
        ...
        it("cannot set weight for zero address distributor", async()=>{
            ...
            await expect(pool_proxy.connect(alice).set_distributor_weight(tokenA.address, 0, 1000)).to.revertedWith("distributor not set");
        });
    });
});