...
describe("GaugeController", function () {
  ...
  describe("test_gauges_weights", function () {
    ...
    it("test_n_gauge_types", async () => {
      ...
      expect(await gauge_controller.n_gauge_types()).to.equal(
        BigNumber.from("3")
      ); //unset, LiquidityGauge, Insurance
    });
    ...
    it("test_gauge_weight", async () => {
      ...
      expect(await gauge_controller.get_gauge_weight(gauge)).to.equal(
        ten_to_the_19
      );
    });
    ...
    it("test_type_weight", async () => {
      ...
      expect(await gauge_controller.get_type_weight(1)).to.equal(
        TYPE_WEIGHTS[0]
      );
    ...
    it("test_change_type_weight", async () => {
      ...
      expect(await gauge_controller.get_type_weight(2)).to.equal(
        TYPE_WEIGHTS[1]
      );
    });
    ...
    it("test_relative_weight_write", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        let relative_weight = await gauge_controller.gauge_relative_weight(
          three_gauges[i],
          t
        );
