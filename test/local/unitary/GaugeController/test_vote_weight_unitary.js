...
describe('GaugeController', function(){
    ...
    describe("test_vote_weight_unitary", function(){
        ...
        it("test_no_immediate_effect_on_weight", async()=>{ //vote hasn't affected whithin this week;
            await gauge_controller.vote_for_gauge_weights(three_gauges[0], 10000);
            expect(await gauge_controller.gauge_relative_weight(three_gauges[0], (BigNumber.from((await ethers.provider.getBlock('latest')).timestamp)))).to.equal(zero);
        });
        ...
        it("test_remove_vote_no_immediate_effect", async()=>{
            ...
            expect(await gauge_controller.gauge_relative_weight(three_gauges[0], (BigNumber.from((await ethers.provider.getBlock('latest')).timestamp)))).to.equal(ten_to_the_18);
        });

        it("test_effect_on_following_period", async()=>{ //vote has affected after this week;
            await gauge_controller.vote_for_gauge_weights(three_gauges[0], 10000);
