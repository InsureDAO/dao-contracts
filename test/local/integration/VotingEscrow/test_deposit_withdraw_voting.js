...
describe("VotingEscrow", function () {
  ...
  async function rule_create_lock() {
    ...
    if (st_value == 0) {
      ...
      await expect(
        voting_escrow.connect(st_account).create_lock(st_value, unlock_time)
      ).to.revertedWith("Can lock until time in future");
  ...
  async function rule_increase_unlock_time() {
    ...
    let unlock_time = timestamp
      .add(st_lock_duration.mul(WEEK))
      .div(WEEK)
      .mul(WEEK);
  ...
  async function rule_force_unlock() {
    ...
    if (st_account == creator) {
      ...
      if (voting_balances[target_account_n]["value"].eq(BigNumber.from("0"))) {
        ...
        console.log("--reverted");
        await expect(
          manager.connect(st_account).force_unlock(target_account.address)
        ).to.revertedWith("There is no locked INSURE");
      } else {
        console.log("--success_force_unlock, account:", target_account_n);
        tx = await manager
          .connect(st_account)
          .force_unlock(target_account.address);
        await tx.wait();
