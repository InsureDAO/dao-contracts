...
describe("LiquidityGauge", function () {
  ...
  describe("test_liquidity_gauge", function () {
    it("test_gauge_integral", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (is_withdraw) {
          ...
          console.log(
            "Bob Withdraws " +
              amount.div(BigNumber.from("10").pow("18")).toNumber()
          );
          ...
          console.log(
            "--------------------> bob:" +
              bob_staked.div(BigNumber.from("10").pow("18")).toNumber()
          );
        } else {
          //deposit
          let amount = BigNumber.from(
          ...
          await mock_lp_token
            .connect(bob)
            .approve(liquidity_gauge.address, amount);
        ...
        if (is_alice) {
          ...
          let is_withdraw_alice =
            (await liquidity_gauge.balanceOf(alice.address)) > 0 &&
            Math.random() > 0.5;
        ...
        expect(await liquidity_gauge.balanceOf(bob.address)).to.equal(
          bob_staked
        );
        ...
        console.log(
          i +
            ...
            integral +
            ":" +
            (await liquidity_gauge.integrate_fraction(alice.address))
        ); //approx 1e-20
      }
    });
    ...
    it("test_mining_with_votelock", async () => {
      ...
      await mock_lp_token
        .connect(bob)
        .approve(liquidity_gauge.address, MAX_UINT256);
      ...
      await voting_escrow.create_lock(
        ten_to_the_20,
        t.add(WEEK.mul(BigNumber.from("2")))
      );
      ...
      expect(
        await voting_escrow["balanceOf(address,uint256)"](alice.address, now)
      ).to.not.equal(zero);
      expect(
        await voting_escrow["balanceOf(address,uint256)"](bob.address, now)
      ).to.equal(zero);
      ...
      let rewards_alice = await liquidity_gauge.integrate_fraction(
        alice.address
      );
      ...
      ).to.equal(BigNumber.from("25000000000000000")); //approx = 1e-16
