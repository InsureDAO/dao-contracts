...
async function main() {
  ...
  const { OwnershipAddress, GovOwnershipAddress } = require("./deployments.js");

  const { ReportingAddress } = require("./config.js");
  ...
  //===deploy start===
  //PoolProxy
  const pool_proxy = await PoolProxy.deploy(deployer.address, deployer.address, deployer.address); //PoolProxy => Deployer
